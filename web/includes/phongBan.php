<!DOCTYPE HTML>
<head>
	<meta http-equiv="content-type" content="text/html" charset="utf-8"/>
	<meta name="a  uthor" content="dai" />
	<link rel="stylesheet" type="text/css" href="structure.css" />
	<title>Cơ Cấu Tổ Chức</title>
    
</head>
  
<html>
<body>
    <?php
        require("employee.php"); 
    ?>
	<div id="box">
		<div id="box-left">
			<ul id="menu">
				<li><a href = "user_structure.php"><b>CƠ CẤU TỔ CHỨC</b></a></li>
				<li><a href = "user_hoiDong.php">Hội đồng thành viên</a></li>
				<li><a href = "user_banGD.php">Ban giám đốc</a></li>
				<li><a href = "user_phongKD.php">Phòng kinh doanh</a></li>
				<li><a href = "user_tcHC.php">Tổ chức hành chính</a></li>
				<li><a href = "user_keToan.php">Kế toán</a></li>
				<li><a href = "user_kyThuat.php">Kỹ thuật</a></li>
			</ul>
		</div>
		<div id="box-right">
			<div id="produce">
				<ul id = "title">
					<li> <b><u>II. CƠ CẤU</u></b></li>
						<ul id="list">
							<li id="hoiDong"><b>BAN GIÁM ĐỐC : </b>Gồm 01 giám đốc điều hành và 2 phó giám đốc</li>
						</ul>
				</ul>
			</div>
			<div id="images3"></div>	
		</div>
		<div id="footer">
			<ul id="title">
				<li>Developed by A4DSE, 2014, Unversity of Techology and Engineering , 
				E3 144 Xuân Thủy - Cầu Giấy - Hà Nội,
				Email: a4dse1994@gmail.com</li>
			</ul>
		</div>
	</div>
</body>
</html>